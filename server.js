...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    leaves.forEach((leave) => {
      const excelStartDate = new Date(Date.UTC(1899, 11, 30)); // Use UTC to ensure consistency
      const leaveStartDate = new Date(
        excelStartDate.getTime() + leave.LeaveStartDate * 86400000
      );
      ...
      for (let i = 0; i < Math.ceil(leave.NumberOfDays); i++) {
        ...
        date.setUTCDate(leaveStartDate.getUTCDate() + i); // Always use UTC for consistency
    ...
    userDayLogs.forEach((entry) => {
      ...
      } else if (leaveType === "half" && dailyLogInMinutes < 180) {
        filteredUsers.push(entry);
      }
    });
